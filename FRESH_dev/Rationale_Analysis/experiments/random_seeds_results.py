...
def main_lei(args):
    ...
    rationale = ["top_k"]
    values = []
    for r, seed in product(rationale, seeds):
        path = os.path.join(
            args.output_dir,
            args.dataset,
            "bert_encoder_generator" if args.lei else "bert_kuma_encoder_generator",
            str(defaults[args.dataset]["MAX_LENGTH_RATIO"]),
        ...
        if os.path.isfile(metrics_file_direct):
            ...
            values.append({"dataset": args.dataset, "thresholder": r, "seed": seed, "value": metrics})
        else :
...
def main_ours(args):
    ...
    for seed in seeds:
        path = os.path.join(args.output_dir, args.dataset, "bert_classification", "random_seed_variance", "RANDOM_SEED=" + str(seed))
    ...
    for s, r, seed in product( saliency, rationale, seeds):
        path = os.path.join(
            args.output_dir,
            args.dataset,
            "bert_classification",
